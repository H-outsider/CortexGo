...
- Provider 增加 GLM、DeepSeek、Qwen、Groq 端点预设和统一工厂，模型协议仍由 OpenAI-compatible 适配层统一处理。
- `internal/observability` 统一模型遥测与执行事件，新增 Langfuse ingestion sink 和多后端 fan-out。
...
安全层已提供 API Key/JWT、RBAC、租户隔离包装器、SSRF/上传校验和审计脱敏；生产环境仍需接入统一身份服务、密钥托管和策略中心。
